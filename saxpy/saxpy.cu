...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double startkernel_Time = CycleTimer::currentSeconds();
    ...
    double endkernel_Time = CycleTimer::currentSeconds();
    ...
    printf("Time taken to run the Kernel: %.7f ms \n", endkernel_Time-startkernel_Time);
